...
RDEPEND="
	...
	ospray? (
		>=media-gfx/ospray-2.4.0
		=media-gfx/oidn-1.2.4
	)
...
src_configure() {
	...
	local mycmakeargs=(
		...
		if use nvcontrol; then
			...
			-DOPENGL_egl_LIBRARY:FILEPATH=/usr/$(get_libdir)/libEGL_nvidia.so
			-DOPENGL_gles2_LIBRARY="${EPREFIX}"/usr/$(get_libdir)/libGLESv2.so
			-DOPENGL_gl_LIBRARY="${EPREFIX}"/usr/$(get_libdir)/libGL.so
			-DOPENGL_glu_LIBRARY="${EPREFIX}"/usr/$(get_libdir)/libGLU.so
			-DOPENGL_opengl_LIBRARY="${EPREFIX}"/usr/$(get_libdir)/libOpenGL.so
			-DOPENGL_glx_LIBRARY="${EPREFIX}"/usr/$(get_libdir)/libGLX_nvidia.so
