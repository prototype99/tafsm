...
SRC_PATCHES="mirror://gentoo/${PN2}-7.10.1-gentoo-patches-01.tar.bz2"
if [[ $PV = 9999* ]]; then
	SRC_URI="${SRC_PATCHES}"
else
...
IUSE="+classic debug +gallium gles llvm motif +nptl pic selinux kernel_FreeBSD hardened"
...
#insopts -m0755
	dolib "${S}/$(get_libdir)/libOSMesa.so" || die "failed to install "
	dolib "${S}/$(get_libdir)/libOSMesa.so.7" || die "failed to install "
	dolib "${S}/$(get_libdir)/libOSMesa.so.7.10.3" || die "failed to install "
