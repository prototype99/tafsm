...
src_compile(){
	...
	emake -j1 SLEPC_DIR=${S} PETSC_DIR=${PETSC_DIR} PETSC_ARCH=${PETSC_ARCH}
	elog "src_compile end"
}
src_install(){
	emake -j1 SLEPC_DIR=${S} PETSC_DIR=${PETSC_DIR} PETSC_ARCH=${PETSC_ARCH} SLEPC_DESTDIR=${D}/usr/share/${P} install
	dolib arch-installed-petsc/lib/libslepc.a
	dolib.so arch-installed-petsc/lib/libslepc.so
}
